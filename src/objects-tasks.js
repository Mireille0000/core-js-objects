...
function mergeObjects(objects) {
  ...
  for (let i = 0; i < objects.length; i += 1) {
    ...
    for (let j = 0; j < keys.length; j += 1) {
      result[keys[j]] =
        objects[0][keys[j]] + (objects[1][keys[j]] || 0) || objects[1][keys[j]];
    }
  }
  return result;
}
...
function removeProperties(obj, keys) {
  ...
  if (typeof keys === 'string') {
    delete resultObj[keys];
  }

  for (let i = 0; i < objKeys.length; i += 1) {
    if (objKeys.includes(keys[i])) {
      delete resultObj[keys[i]];
    }
  }
  return resultObj;
}
...
function compareObjects(obj1, obj2) {
  const objToStr1 = JSON.stringify(obj1);
  const objToStr2 = JSON.stringify(obj2);
  return objToStr1 === objToStr2;
}
...
function sellTickets(queue) {
  ...
  const moneyForChange = queue.reduce((acc, ticket) => {
    let accum = acc;
    if (queue.indexOf(ticket) !== queue.length - 1) {
      accum += ticket;
    }
    return accum;
  }, 0);

  return moneyForChange >= queue[queue.length - 1];
}
...
function fromJSON(proto, json) {
  ...
  return Object.setPrototypeOf(jsonToObj, proto);
}
